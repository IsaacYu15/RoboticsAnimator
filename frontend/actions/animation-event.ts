...
export async function addAnimationEvent(data: CreateAnimationEventInput) {
  try {
    ...
    revalidatePath(`/animate/animation/${newEvent.animation_id}`);
...
export async function updateAnimationEvent(
  ...
  try {
    ...
    revalidatePath(`/animate/animation/${updated.animation_id}`);
...
export async function deleteAnimationEvent(id: number) {
  try {
    const deleted = await prisma.animation_events.delete({
      where: { id },
    });
    revalidatePath(`/animate/animation/${deleted.animation_id}`);
    return { success: true };
  } catch (error) {
    console.error(error);
